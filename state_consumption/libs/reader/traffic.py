...
class ConsumptionTrafficReader(Reader):
    ...
    def get_data(self) -> pd.DataFrame:
        try:
            ...
            logger.error(
                f"No se ha podido obtener la data de consumo del archivos de BRAS - {error}"
            )
            terminal.print_spinner(
                f"[red3]ERROR: [default]No se ha podido obtener la data de consumo del archivo de BRAS - {error}"
            )
            exit(1)
        else:
            return df
    ...
    def check_reader(self) -> bool:
        try:
            ...
            logger.error(
                f"Problemas en la comprobación de la data de consumo del archivo de BRAS - {error}"
            )
            terminal.print_spinner(
                f"[red3]ERROR: [default]Problemas en la comprobación de la data de consumo del archivo de BRAS - {error}"
            )
            return False
        else:
            return True
