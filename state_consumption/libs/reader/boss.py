import pandas as pd
from state_consumption.constants import NameColumns, PathStderr, BossNewNameColumns, BossNameColumns, StatusClients, boss_all_columns
...
class BossReader(Reader):
    ...
    def __init__(self, filename: str, dev: bool = False, testing: bool = False):
        if dev: self._database = MongoDatabase(dev=True)
        elif testing: self._database = MongoDatabase(testing=True)
        else: self._database = MongoDatabase()
    ...
    def _add_state(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for account_code in list_account_code:
            ...
            if state: df.loc[mask, NameColumns.STATE] = state
        return df
