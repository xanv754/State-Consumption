...
from state_consumption.libs import (
    AsfReader,
    BossReader,
    ...
    def consumtion_bras(self, filepath: str | None = None) -> str | None:
        ...
        try:
            consumption = ConsumptionTrafficReader(
                path=self.bras_path, process=self.process_consumption
            )
            ...
            logger.error(
                f"Ha ocurrido un error al procesar la data de consumo - {error}"
            )
            terminal.print(
                f"[red3]ERROR: [default]Ha ocurrido un error al procesar la data de consumo - {error}"
            )
            return None
    ...
    def clients_consumption_by_state(self, filepath: str | None = None) -> str | None:
        ...
        try:
            if self.boss_path is None or self.asf_path is None:
                logger.error(
                    "Se ha intentado procesar el consumo por estado, pero no se ha encontrado data en el archivo BOSS o ASF"
                )
                terminal.print(
                    "[red3]ERROR: [default]Se ha intentado procesar el consumo por estado, pero no se ha encontrado data en el archivo BOSS o ASF"
                )
                return None
            process = ProcessData(
                self.boss_path, self.bras_path, self.asf_path, self.process_consumption
            )
            ...
            logger.error(
                f"Ha ocurrido un error al generar el consumo por estado - {error}"
            )
            terminal.print(
                f"[red3]ERROR: [default]Ha ocurrido un error al generar el consumo por estado - {error}"
            )
            return None

    def clients_consumption_by_state_with_percentage(
        ...
        try:
            ...
            logger.error(
                f"Ha ocurrido un error al generar el consumo por estado con porcentaje - {error}"
            )
            terminal.print(
                f"[red3]ERROR: [default]Ha ocurrido un error al generar el consumo por estado con porcentaje - {error}"
            )
            return None
