...
class ReportHandler:
    ...
    def _merge_consumption_by_all_states(
        self, df_clients: pd.DataFrame, df_consumption: pd.DataFrame
    ) -> pd.DataFrame:
        """Merge the data comsuption and clients (without percentage) by all states."""
        try:
            df_merge = pd.merge(
                df_clients, df_consumption, on=NameColumns.STATE, how="inner"
            )
            ...
            df[[NameColumns.TOTAL_CLIENTS, NameColumns.CONSUMPTION]] = df[
                [NameColumns.TOTAL_CLIENTS, NameColumns.CONSUMPTION]
            ].fillna(0)
    ...
    def _merge_percentage_by_all_states(
        self, df_data: pd.DataFrame, df_percentage: pd.DataFrame
    ) -> pd.DataFrame:
        """Merge the data percentage and data (without percentage) by all states."""
        try:
            df_merge = pd.merge(
                df_data, df_percentage, on=NameColumns.STATE, how="left"
            )
            df_merge = df_merge.reset_index(drop=True)
            df_merge[[NameColumns.PERCENTAGE_CONSUMPTION]] = df_merge[
    ...
    def clients_consumption_adsl_by_state(self) -> pd.DataFrame:
        ...
        try:
            ...
            df = self._merge_consumption_by_all_states(
                df_total_clients, df_total_consumption
            )
            df.rename(
                columns={
