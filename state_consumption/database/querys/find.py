...
class FindQuery:
    ...
    def find_state_by_central(self, central: str) -> str | None:
        ...
        """
        try:
            response = self._collection.find_one({NodesField.CENTRAL: central})
            if response:
                return response[NodesField.STATE]
            return None
    ...
    def find_state_by_cc(self, cc: str) -> str | None:
        ...
        """
        try:
            response = self._collection.find_one({NodesField.ACCOUNT_CODE: cc})
            if response:
                return response[NodesField.STATE]
            return None
        except Exception as error:
