...
class ExcelExport:
    ...
    def _add_styles(self, sheet_name: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"No se ha podido añadir los estilos al archivo de excel - {error}"
            )
            terminal.print_spinner(
                f"[red3]ERROR: No se ha podido añadir los estilos al archivo de excel - {error}"
            )
            return False
        else:
            return True
    ...
    def _add_total(self, sheet_name: str, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if ConsumptionStateColumns.TOTAL_CLIENTS_ADSL in data.columns.to_list():
                total_clients_adsl = data[
                    ConsumptionStateColumns.TOTAL_CLIENTS_ADSL
                ].sum()
                total_clients += total_clients_adsl
                column = data.columns.get_loc(
                    ConsumptionStateColumns.TOTAL_CLIENTS_ADSL
                )
            ...
            if ConsumptionStateColumns.CONSUMPTION_ADSL in data.columns.to_list():
                total_consumption_adsl = data[
                    ConsumptionStateColumns.CONSUMPTION_ADSL
                ].sum()
                total_consumption += total_consumption_adsl
                column = data.columns.get_loc(ConsumptionStateColumns.CONSUMPTION_ADSL)
                sheet.cell(row=max_row, column=column + 1).value = (
                    total_consumption_adsl
                )
                sheet.cell(row=max_row, column=column + 1).border = border
            if (
                ConsumptionStateColumns.PERCENTAGE_CONSUMPTION_ADSL
                in data.columns.to_list()
            ):
                total_percentage_consumption_adsl = data[
                    ConsumptionStateColumns.PERCENTAGE_CONSUMPTION_ADSL
                ].sum()
                ...
                sheet.cell(row=max_row, column=column + 1).value = (
                    total_percentage_consumption_adsl
                )
                sheet.cell(row=max_row, column=column + 1).border = border
            if ConsumptionStateColumns.TOTAL_CLIENTS_MDU in data.columns.to_list():
                total_clients_mdu = data[
                    ConsumptionStateColumns.TOTAL_CLIENTS_MDU
                ].sum()
            ...
            if ConsumptionStateColumns.CONSUMPTION_MDU in data.columns.to_list():
                total_consumption_mdu = data[
                    ConsumptionStateColumns.CONSUMPTION_MDU
                ].sum()
                ...
                sheet.cell(row=max_row, column=column + 1).border = border
            if (
                ConsumptionStateColumns.PERCENTAGE_CONSUMPTION_MDU
                in data.columns.to_list()
            ):
                total_percentage_consumption_mdu = data[
                    ConsumptionStateColumns.PERCENTAGE_CONSUMPTION_MDU
                ].sum()
                ...
                sheet.cell(row=max_row, column=column + 1).value = (
                    total_percentage_consumption_mdu
                )
                sheet.cell(row=max_row, column=column + 1).border = border
            if ConsumptionStateColumns.TOTAL_CLIENTS_OLT in data.columns.to_list():
                total_clients_olt = data[
                    ConsumptionStateColumns.TOTAL_CLIENTS_OLT
                ].sum()
            ...
            if ConsumptionStateColumns.CONSUMPTION_OLT in data.columns.to_list():
                total_consumption_olt = data[
                    ConsumptionStateColumns.CONSUMPTION_OLT
                ].sum()
                ...
                sheet.cell(row=max_row, column=column + 1).border = border
            if (
                ConsumptionStateColumns.PERCENTAGE_CONSUMPTION_OLT
                in data.columns.to_list()
            ):
                total_percentage_consumption_olt = data[
                    ConsumptionStateColumns.PERCENTAGE_CONSUMPTION_OLT
                ].sum()
                ...
                sheet.cell(row=max_row, column=column + 1).value = (
                    total_percentage_consumption_olt
                )
                sheet.cell(row=max_row, column=column + 1).border = border
            ...
            if NameColumns.PERCENTAGE_CONSUMPTION in data.columns.to_list():
                total_percentage_consumption_data = data[
                    NameColumns.PERCENTAGE_CONSUMPTION
                ].sum()
                column = data.columns.get_loc(NameColumns.PERCENTAGE_CONSUMPTION)
                sheet.cell(row=max_row, column=column + 1).value = (
                    total_percentage_consumption_data
                )
                sheet.cell(row=max_row, column=column + 1).border = border
            ...
            if total_consumption != 0:
                sheet.cell(row=max_row + 5, column=1).value = (
                    "Total de Consumo Procesado"
                )
            ...
            if total_percentage_consumption != 0:
                sheet.cell(row=max_row + 6, column=1).value = (
                    "Porcentage de Consumo Procesado"
                )
                sheet.cell(row=max_row + 6, column=1).border = border
                sheet.cell(row=max_row + 6, column=2).value = (
                    total_percentage_consumption
                )
            ...
            terminal.print_spinner(
                f"[red3]ERROR: [default]No se ha podido añadir los totales al excel - {error}"
            )
            return False
        else:
            return True
    ...
    def export(self, data: pd.DataFrame, sheet_name: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"No se ha podido exportar el archivo {self.filepath} - {error}"
            )
            terminal.print_spinner(
                f"[red3]ERROR: [default]No se ha podido exportar el archivo {self.filepath} - {error}"
            )
            return False
        else:
            return True
    ...
    def add(self, new_data: pd.DataFrame, sheet_name: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"No se ha podido añadir una nueva hoja al archivo {self.filepath} - {error}"
            )
            terminal.print_spinner(
                f"[red3]ERROR: [default] No se ha podido añadir una nueva hoja al archivo {self.filepath} - {error}"
            )
            return False
        else:
            return True
