    ...
    def get_database(self) -> MongoDatabase:
        try:
            ...
            terminal.print_spinner(
                f"[red3]ERROR: [default]Unit test error. Problema con la base de datos - {error}"
            )
            exit(1)
...
class ConsumptionFile(FileHandler):
    ...
    def get_example(self) -> Dict[str, list[str | float]]:
        return {
            NameColumns.BRAS: [
                "CNT-BRAS-00_Huawei_10GB_GE_100",
                "CNT-BRAS-00_Huawei_10GB_GE_101",
            ],
