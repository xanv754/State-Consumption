...
from common.constant import export, exception, states, file
...
class DatabaseController:
    ...
    def _add_missing_nodes(self, index: int, central: str) -> None:
        node = {export.INDEX: index + 2, export.CENTRAL: central}
    ...
    def export_missing_nodes(self) -> None:
        try:
            if len(self.missing_nodes) > 0:
                export_missing_nodes(self.missing_nodes, file.MISSING_NODES)
        except Exception as error:
            raise error
    ...
    def save_new_node(
        ...
        try:
            if find_node(state, central, account_node):
                return True
            else:
                new_node = Node(
                    state=state,
